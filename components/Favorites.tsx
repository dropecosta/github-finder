...
const Favorites: React.FC<FavoritesProps> = ({ data, repos }) => {
  ...
  // console.log('Favorites repos', repos);
  // console.log('Transform repos', repos.items.map((item: { id: any; }, index: string | number) => (item.id[index])));
  // console.log('fav item', item);



  // if(data == repos.items.map((item: { id: any; }) => (item.id).toString())) {
  ...
  return (<>
    ...
    <div className="flex justify-end z-20 absolute right-0 pr-2 pt-2">
      ...
      <a className="h-full w-full aspect-square block absolute top-0 left-0 transition-opacity duration-30 z-10 font-bold text-gray-800 mb-2">
        ...
        <h3 className="text-white py-2 px-3 mx-auto">stars: 565</h3>
      </a>
    </div>
